...
static int txt_serialize_response(struct thingset_context* ts, uint8_t code, char const* msg, ...) {
    va_list vargs;
...
static int txt_serialize_value(struct thingset_context* ts,
    ...
    if (pos == -THINGSET_ERR_UNSUPPORTED_FORMAT) {
        ...
        else if (((object->type == THINGSET_TYPE_FN_VOID) ||
                  (object->type == THINGSET_TYPE_FN_I32 ))) {
            pos = snprintf(buf, size, "[");
...
static int txt_serialize_string(struct thingset_context* ts, char const* buf, bool is_key) {
...
static int txt_deserialize_value(struct thingset_context* ts,
    ...
    if ((err == -THINGSET_ERR_UNSUPPORTED_FORMAT) &&
        (object->type == THINGSET_TYPE_ARRAY)) {
        struct thingset_array* array = object->data.array;
...
static int txt_serialize_subsets(struct thingset_context* ts, uint16_t subsets) {
    ...
    for (unsigned int i = 0; i < ts->num_objects; i++) {
        if (ts->data_objects[i].subsets & subsets) {
            uint16_t const parent_id = ts->data_objects[i].parent_id;
...
static int txt_deserialize_string(struct thingset_context* ts, char const** str_start,
                                  size_t* str_len) {
    if (ts->tok_pos < ts->tok_count) {
        if (ts->tokens[ts->tok_pos].type == JSMN_STRING) {
...
static struct thingset_api const txt_api = {
