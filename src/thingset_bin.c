    ...
    if (object->type == THINGSET_TYPE_GROUP) {
        ...
        for (int i = 0; i < array->num_elements; i++) {
            ...
            union thingset_data_pointer data = {
                .u8 = array->elements.u8 + (i * type_size)
            };
            err =
...
static int bin_parse_endpoint(struct thingset_context* ts) {
    ...
    if (err != 0) {
        ts->api->serialize_response(ts, (uint8_t)-err, "Invalid endpoint");
        return (err);
    }
...
static int bin_serialize_report_header(struct thingset_context* ts, char const* path) {
    bool success;
        ...
        switch (object->type) {
            case THINGSET_TYPE_ARRAY :
                ...
                do {
                    /* using uint8_t pointer for byte-wise pointer arithmetics */
                    union thingset_data_pointer data = {
                        .u8 = array->elements.u8 + (index * type_size)
                    };
            ...
            case THINGSET_TYPE_RECORDS :
                ...
                for (unsigned int i = 0; i < records->num_records; i++) {
                    ...
                    while (zcbor_uint32_decode(ts->decoder, &id) && id < UINT16_MAX) {
                        struct thingset_data_object const* element = thingset_get_object_by_id(ts, id);
                        if (element == NULL) {
                        ...
                        union thingset_data_pointer data = {
                            .u8 = ((uint8_t*)records->records) + (i * records->record_size) + element->data.offset
                        };
...
static int bin_deserialize_finish(struct thingset_context const* ts) {
    zcbor_state_t const* decoder = ts->decoder;

    return ((decoder->payload_end == decoder->payload) ? 0 : -THINGSET_ERR_BAD_REQUEST);
}

static struct thingset_api const bin_api = {
